...
class Encoder(nn.Module):
    def __init__(self, img_size=64, channel_in=3, z_size=512):
...
class VaeGan(nn.Module):
    ...
    def forward(self, x, gen_size=10):
        if self.training:
            ...
            # print(x_tilde.shape,111)  [32, 3, 64, 64]

            z_p = Variable(torch.randn(len(x), self.z_size).cuda(), requires_grad=True)
            x_p = self.decoder(z_p)
            # x 是原始图像  x_tilde 是根据原始图像生成的  x_p是随机抽取lantant codes生成的
            ...
            # print(disc_layer.shape, 111)  [96, 16384]  中间层
            # print(disc_class.shape, 222)  [96, 1]      标签层
    ...
    def loss(x, x_tilde, disc_layer_original, disc_layer_predicted, disc_layer_sampled, disc_class_original,
        ...
        :param x: 输入图像
        :param x_tilde: 生成图像
        :param disc_layer_original: 判别器特征  输入原图像
        :param disc_layer_predicted: 判别器特征  输入重建图像
        :param disc_layer_sampled: 判别器特征  输入生成图像
        :param disc_class_original: 判别器结果  输入原图像
        :param disc_class_predicted: 判别器结果  输入重建图像
        :param disc_class_sampled: 判别器结果 输入生成图像
        :param mus: μ
        :param variances: d
        :return:
