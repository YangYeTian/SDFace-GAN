...
class Generator(nn.Module):
    ...
    Args:
        ...
        resolution_vol (int): resolution of volume-rendered image  渲染图像分辨率
    ...
    def forward(self, batch_size=32, latent_codes=None, camera_matrices=None,
        ...
        if latent_codes is None:
            ...
            # print(latent_codes[0].shape, 000)  [32, 1, 256]
            # print(latent_codes[1].shape, 111)  [32, 1, 256]
            # print(latent_codes[2].shape, 222)  [32, 128]
            # print(latent_codes[3].shape, 333)  [32, 128]
        ...
        if z is not None:
            ...
            z_s = z[..., :self.z_dim]
            z_a = z[..., self.z_dim:]
            latent_codes = (z_s, z_a, latent_codes[2], latent_codes[3])

        # 生成相机矩阵ξ
        if camera_matrices is None:
            camera_matrices = self.get_random_camera(batch_size)

        # 使用bounding_box生成器  生成对应的s t R
    ...
    def get_latent_codes(self, batch_size=32, tmp=1.):
        ...
        # n_boxes 指除去背景一共有几个对象，celeba为一个
    ...
    def get_vis_dict(self, batch_size=32):
        ...
        vis_dict = {
            ...
            # [16, 1, 64]
            'latent_codes': self.get_latent_codes(batch_size),
            # 获取相机变换矩阵，celeba使用的矩阵相同，即固定相机视角
            'camera_matrices': self.get_random_camera(batch_size),
            # 从bounding_box_generator 中获取对象的变换矩阵s, t, R
            'transformations': self.get_random_transformations(batch_size),
            #
            'bg_rotation': self.get_random_bg_rotation(batch_size)
        }
    ...
    def get_random_camera(self, batch_size=32, to_device=True):
        # 生成随机相机参数
        # celeba数据集中，两个变换矩阵都是一致的，即不改变相机方向，仅旋转对象
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
        ...
        for i in range(n_iter):
            if i < n_boxes:  # Object
                # 获取点p和视角r  [32, 16384, 3]  [32, 16384, 3]
                ...
                # print(sigma_i.shape, "sigma")  ([32, 256, 64])
