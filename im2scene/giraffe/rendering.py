...
from extract_mesh import export_mesh
...
class Renderer(object):
    ...
    def render_full_visualization(self, img_out_path,
        ...
        for rp in render_program:
            if rp == 'object_rotation':
                ...
                self.render_object_rotation(img_out_path, batch_size=batch_size,data=data)
    ...
    def render_object_rotation(self, img_out_path, batch_size=15, n_steps=32, data=None):
        gen = self.generator
        ...
        for step in range(n_steps):
            ...
            with torch.no_grad():
                ...
                out_i = gen(batch_size, latent_codes, camera_matrices,
                            transformations, bg_rotation, mode='val', z=z)
        ...
        # 生成mesh
        grid_Size = torch.tensor([121, 215, 128]).to(self.device)
        z_s = latent_codes[0]
        i = 0
        for z in z_s:
            z = z.unsqueeze(dim=1)  # 增加一个维度
            ply_filename_out = "out/ffhq256_pretrained/rendering/rotation_object/" + str(i) + "test.ply"
            export_mesh(gen.decoder, grid_Size, ply_filename_out, z, self.device)
            i += 1
